...
@section('content')
            ...
            <div class="rounded-2xl border border-gray-200 bg-white pt-4">
                ...
                <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <form method="GET" action="{{ route('panitia.events.index') }}"
                            class="flex flex-col gap-3 sm:flex-row items-center gap-x-4">
                            ...
                            <div x-data="{
                                ...
                                get isOptionSelected() { return this.selectedSort !== ''; }
                            }" class="w-full sm:w-fit">
                                <div class="relative z-20 bg-transparent ">
                                    <select x-model="selectedSort" name="sort" @change="$el.form.submit()"
                                        class="w-full sm:w-fit dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 h-[42px]  appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 text-sm placeholder:text-gray-400 focus:ring-3 focus:outline-hidden"
                                ...
                                @forelse ($events as $event)
                                    <tr>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <!-- Container Utama: Menggunakan w-28 (lebar) dan aspect-video (tinggi otomatis 16:9) -->
                                            <div
                                                class="relative w-32 aspect-video overflow-hidden rounded-md border border-gray-200 bg-gray-50 group">

                                                @if ($event->gambar)
                                                    <!-- Tampilan jika gambar ada -->
                                                    <img src="{{ Storage::url($event->gambar) }}"
                                                        alt="{{ $event->nama }}"
                                                        class="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                                                        loading="lazy">
                                                @else
                                                    <!-- Tampilan jika gambar kosong / placeholder -->
                                                    <div class="flex h-full w-full items-center justify-center">
                                                        <div class="text-center">
                                                            <svg xmlns="http://w3.org" width="18" height="18"
                                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                                stroke-width="2" stroke-linecap="round"
                                                                stroke-linejoin="round"
                                                                class="lucide lucide-image-off mx-auto text-gray-400">
                                                                <line x1="2" x2="22" y1="2"
                                                                    y2="22" />
                                                                <path d="M10.41 10.41a2 2 0 1 1-2.83-2.83" />
                                                                <line x1="13.5" x2="6" y1="13.5"
                                                                    y2="21" />
                                                                <line x1="18" x2="21" y1="12"
                                                                    y2="15" />
                                                                <path
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500">
                                                {{ $event->tanggal->translatedFormat('d M Y, H:i') }}
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500">{{ $event->kuota }}
                                                {{ __('messages.orang') }}
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500">{{ $event->sisa_kuota }}
                                                {{ __('messages.orang') }}
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <div class="flex justify-center relative">
                                                <x-common.table-dropdown>
                                                    <x-slot name="button">
                                                        <button type="button" id="options-menu-{{ $event->id }}"
                                                    ...
                                                    <x-slot name="content">
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            nama: @js($event->nama),
                                                            url: '{{ route('panitia.events.destroy', $event) }}'
                                                        })"
    ...
    <x-ui.modal class="max-w-[400px] p-6 lg:p-8">
        <div x-data="{ eventNama: '', deleteUrl: '' }"
            @open-delete-modal.window="
                open = true;
                eventNama = $event.detail.nama;
            ...
            <div class="text-center">
                <div class="mb-5 flex justify-center">
                    <div class="flex h-14 w-14 items-center justify-center rounded-full bg-red-50">
                        <svg class="text-red-500" width="28" height="28" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 3h6m-9 4h12M6 7l1 13a2 2 0 002 2h6a2 2 0 002-2l1-13M10 11v6m4-6v6"
                ...
                <h4 class="mb-2 text-lg font-semibold text-gray-800">Hapus Event</h4>
                <p class="mb-6 text-sm text-gray-500">
                    Yakin ingin menghapus
                    <span class="font-medium text-gray-700" x-text="eventNama"></span>?
